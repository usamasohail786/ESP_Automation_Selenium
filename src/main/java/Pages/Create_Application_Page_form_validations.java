...
public class Create_Application_Page_form_validations extends Test_Data {
	
	WebDriver driver = null;
	@FindBy(xpath="//h4[@class='group-title']")
	List<WebElement> choose_group;
	@FindBy(xpath="//input[@data-type]")
	List<WebElement>list_of_fields ;
	@FindBy(xpath="//mat-error")
	List<WebElement>validations_list ;
    ...
    public Create_Application_Page_form_validations(WebDriver driver) throws FileNotFoundException, IOException, ParseException {
...
//    	com.Explicit_wait_elementToBeVisible(content_load, 50);
    ...
    public String verify_validation_on_min_text_all_short_text() throws InterruptedException
    {
    	//form submitting in all short text
    	Common_class com=new Common_class(driver);
    	String validations = null;
        com.Explicit_wait_elements_visiblity(list_of_fields, 10);
        com.get_elements_text_random_validation(text_type_list);
        System.out.print(text_type_list.size()+ "Size of list");
        int size_of_min_short_text_expect=list_of_fields.size()-text_type_list.size();
        int size_of_min_text_fields=list_of_fields.size()-size_of_min_short_text_expect;
        System.out.print(size_of_min_text_fields);
        if(size_of_min_text_fields==text_type_list.size())
        {
        	 validations="Validation on short text working fine";
        }
        else
        {
        	validations="Validation on short text not working fine";
        }
        return validations;
        	
    }
   
    public String verify_validation_on_min_text_all_email_text() throws InterruptedException
    {
    	Common_class com=new Common_class(driver);
    	String validations = null;
        com.Explicit_wait_elements_visiblity(list_of_fields, 10);
        com.randomemailFromArrValidation(text_type_email_list);
        System.out.print(text_type_email_list.size()+ "Size of list");
        int size_email_field_expect=list_of_fields.size()-text_type_email_list.size();
        int size_of_email_fields=list_of_fields.size()-size_email_field_expect;
        System.out.print(size_of_email_fields);
        if(size_of_email_fields==text_type_email_list.size())
        {
        	 validations="Validation on Email  working fine";
        }
        else
        {
        	validations="Validation Email not working fine";
        }
        return validations;
